...
sample_forest <- function() {
  ...
  pinguino_forest_cv <- rand_forest(trees = 200, mode = "classification") %>% 
    set_engine("randomForest") %>% 
    fit(species ~ culmen_length + culmen_depth + body_mass + island + sex, data = pinguino_training_cv)
...
sample_knn <- function() {
  ...
  pinguino_knn_cv <- nearest_neighbor(mode = "classification", dist_power = 2, weight_func = "optimal") %>% 
    set_engine("kknn") %>% 
    fit(species ~ culmen_length + culmen_depth + body_mass, data = pinguino_training_cv)
...
A simple cross-validation algorithm was developed to determine the accuracy of both models. Both algorithms performed very well with this data set. The performance of the random forests and k-nearest neighbors models is nearly identical. The random forests model consistently performs ever so slightly better (see table below). This stands to reason since the random forests model makes use of two additional categorical predictors, compared with the k-nearest neighbors model.

```{r}
tribble(
  ~model, ~accuracy,
  "random forests", 0.98865,
  "k-nearest neighbors", 0.98788
) %>% 
  knitr::kable()
```

The computational efficiency of each training each model and generating predictions was explored, and no significant difference was observed. (The k-nearest neighbors algorithm was 2.2% faster.) This exploration was not rigorous, however, and no effort was made to separate out the time each took to generate model predictions, which is probably the more meaningful metric.