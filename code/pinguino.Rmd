...
Three different machine learning models (random forest, k-nearest neighbors, and gradient boost) were used to attempt to predict species of penguins from data collected on body measurements. Once each model was fit to training data, the relevant hyperparameters were tuned, and the predictions from the finalized models were compared.
...
The `tidymodels` package was used to perform predictive analysis using random forest and k-nearest neighbors supervised machine learning classification algorithms.
...
```{r gboost_confusion_matrix, out.width = "70%"}
cm <- pinguino_gboost %>%
  predict(pinguino_testing) %>%
  bind_cols(pinguino_testing) %>%
  conf_mat(truth = species, estimate = .pred_class)
...
tribble(
  ~model, ~accuracy,
  "random forest", 0.98606,
  "k-nearest neighbors", 0.99030,
  "gradient boost", 0.99121
) %>%
  knitr::kable()
```
