...
**Culmen length**, **culmen depth**, **flipper length**, **body weight**, **sex**, and **island** (6) can all be used as predictors for species.
...
```{r}
pinguinos %>% 
  ggplot(aes(flipper_length, body_weight)) +
    geom_point() +
    geom_smooth(
      method = "lm",
      color = "black",
    ...
    labs(
      title = "comparing flipper length and body weight",
      x = "flipper length (mm)",
      y = "body weight (g)"
    )
```
...
```{r}
corr_flipper_to_weight <- pinguinos %>% 
  select(flipper_length, body_weight) %>% 
  filter(!is.na(flipper_length), !is.na(body_weight)) %>% 
  cor() %>% .[[2]]
```

The observed penguin flipper length and body weight show a significant positive correlation (`r format(corr_flipper_to_weight, digits = 3)`). For this reason, we've chosen to use one or the other as a predictor (not both). The chosen predictors are `culmen_depth`, `culmen_length`, `body_weight`, `sex`, and `island`.

<br>

### Predictive Analysis

The `tidymodels` package was used to perform predictive analysis.
...
pinguino_recipe <- training(pinguino_split) %>% 
  recipe(species ~ culmen_length + culmen_depth + body_weight + island + sex) %>% 
  step_center(
    all_numeric(),
    -all_outcomes()
...
pinguino_forest <- rand_forest(trees = 200, mode = "classification") %>% 
  ...
  fit(species ~ culmen_length + culmen_depth + body_weight + island + sex, data = pinguino_training)
```

A **random forest** supervised machine learning classification model (`randomForest` package) was fit to the test data using the `parsnip` package.
