...
The observed penguin flipper length and body mass show a significant positive correlation (**`r format(corr_flipper_to_mass, digits = 3)`**). For this reason, I've chosen to use one or the other as a predictor (not both).
...
A **random forests** classification algorithm (`randomForest` package) was trained on the test data using the `parsnip` package. The number of trees was set to 200.
...
```{r forest_confusion_matrix, eval = FALSE, out.width = "70%"}
cm <- pinguino_forest %>%
  predict(pinguino_testing) %>%
  bind_cols(pinguino_testing) %>%
...
A **weighted k-nearest neighbors** classification algorithm (`kknn` package) was trained on the test data using the `parsnip` package. Euclidean distance was used for distance calculation. The package default of 5 nearest neighbors was left unchanged. The "optimal" kernel weight function was used.

As with the random forests model, the trained model was used to generate predictions from the sampled test data, and the accuracy of the predictions was calculated using the `yardstick` package.

```{r knn_confusion, include = FALSE, out.width = "70%"}
cm_knn <- pinguino_knn %>%
  predict(pinguino_testing) %>%
  bind_cols(pinguino_testing) %>%
  conf_mat(truth = species, estimate = .pred_class)
...
sample_forest <- function() {
  ...
  # train model
  pinguino_forest_cv <- rand_forest(trees = 200, mode = "classification") %>%
    set_engine("randomForest") %>%
...
sample_knn <- function() {
  ...
  # train model
  pinguino_knn_cv <- nearest_neighbor(mode = "classification", dist_power = 2, weight_func = "optimal") %>%
    set_engine("kknn") %>%
    fit(species ~ ., data = pinguino_training_cv)
...
Simple cross-validation was performed to determine the accuracy of both models. Both algorithms performed very well (**99% accuracy**) with this data set. The performance of the random forests and k-nearest neighbors models is nearly identical when both models make use of all of the selected predictors with the hyperparameters used for this analysis.

```{r}
tribble(
  ~model, ~accuracy,
  "random forests", 0.98817,
  "k-nearest neighbors", 0.99053
) %>%
  knitr::kable()
```

The computational efficiency of each training each model and generating predictions was explored, and no significant difference was observed. (The k-nearest neighbors algorithm was 5.37% faster.) This exploration was not rigorous, however, and no effort was made to separate out the time each took to generate model predictions, which is probably the more meaningful metric.
