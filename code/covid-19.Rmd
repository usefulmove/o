...
state_alarm <- 25
count_alarm_states <- sum(state_risk_estimates$risk_estimate >= state_alarm)
  
state_df <- state_risk_estimates %>% # highest risk estimates
  filter(risk_estimate > 5) %>%
  top_n(max(c(count_alarm_states, 15)), risk_estimate)
```
...
A total of `r count_alarm_states` states currently have risk estimates above 25.
...
state_df %>%
  ggplot(
    ...
    geom_bar(
      aes(fill = risk_estimate < state_alarm),
      stat = "identity",
      width = 0.8,
      alpha = 0.8,
      show.legend = FALSE
